...
const Programkeahlian = () => {
  return (
    <>
      <ProgramkeahlianComponent />
    </>
  );
};

export default Programkeahlian;
const ProgramkeahlianComponent = () => {
  const [dataRes, setDataRes] = createSignal(null);
  const [loading, setLoading] = createSignal(true);
  const [error, setError] = createSignal(false);
  const params = useParams();

  // Buat fungsi yang akan mengambil data program keahlian
  const getVisiMisi = async (programkeahlian_id) => {
    try {
      setLoading(true);
      const response = await ApiNode.get(
        `guest/data/programkeahlian/${programkeahlian_id}`
      );
  ...
  // Gunakan `createEffect` untuk memantau perubahan parameter rute
  createEffect(() => {
    // Ambil nilai `id` dari parameter rute
    const programkeahlian_id = params.id;
    // Panggil fungsi `getVisiMisi` untuk mengambil data yang sesuai
    getVisiMisi(programkeahlian_id);
  });
